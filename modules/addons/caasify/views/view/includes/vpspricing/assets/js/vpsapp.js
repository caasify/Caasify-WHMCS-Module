...
vpsapp = createApp({
    ...
    watch: {
        ...
        FilterTermsAreLoaded(newValue){
            if(newValue == true){
                this.loadPlansFromFiltersTermForVPSPricing()
                setTimeout(() => {
                    this.loadRange()
                }, 0.2 * 1000);
            }
        ...
        fileName(newFielName) {
            if (newFielName != null) {
                ...
                } else if (newFielName == "vpspricing.php") {
                    this.loadConfigForVPSPricing()
                }
            }
        },
    ...
    methods: {
        ...
        ScrollTopSelect() {
            const topElement = document.getElementById('topPlaceOnCreate');
            if (topElement) {
                topElement.scrollIntoView({ behavior: 'smooth' });
            }
        },
        ...
        formatPlanPrice(price) {
            ...
            if(price<1){
                ...
                return (Math.floor(price * 10000) / 10000).toFixed(2);
            }
        ...
        formatPlanPriceForVPS(price) {
            if(this.VpsRatio != null){
                price = price * this.VpsRatio
            } 
            ...
            if(price < 0.01){
                ...
                return (Math.floor(price * 10000) / 10000).toFixed(0);
            } else {
                price =(Math.floor(price * 10000) / 10000).toFixed(0)
        ...
        thePlansClass(plan){
            if (this.isPlan(plan)) {
                ...
                return 'bg-primary border-primary text-dark'
        ...
        thePlansStyle(plan){
            if (this.isPlan(plan)) {
                ...
                return '--bs-bg-opacity: 0.01; --bs-border-opacity: 0.15; direction:ltr;'
            }
        },
        ...
        //     const response = await axios.get('/fetch-products.php?action=loadFilterTerms')

        //     if (response?.data == null) {
        //         console.error('Filters are Null');
        ...
        //     if(Object.keys(this.selectedTerms).length === 0){
        //         let formData = new FormData();
        //         // formData.append('term[]', 291)
        //         formData.append('term[][]' , 291);
        //         response = await axios.post('/fetch-products.php?action=GetPlansFromFiltersTerm', formData)                
        //     } else {
        //         let formData = new FormData();
        ...
        //                 let response = 'no data'
        //                 if(this.selectedTerms['Country'] && this.selectedTerms['Country'].length > 0){
        //                     let formData = new FormData();
        //                     for (const term in this.selectedTerms) {
        ...
        //                     response = await axios.post('/fetch-products.php?action=GetRecomPlansForContinent', formData)
        ...
        selectedTermsSave(){
            this.loadPlansFromFiltersTermForVPSPricing()
            setTimeout(() => {
                this.loadRecommendedPlansForContinentForVPSPricing()
            }, 2 * 1000);
            this.SelectedPlan = null
            this.PlanSections = null
        },
        ...
        async loadConfigForVPSPricing(){
            let formData = new FormData();
            formData.append('origin', true);
            const response = await axios.post(this.createRequestLinkForVPSPricing('loadConfigForVPSPricing'), formData)
            this.FinancialParams = null
            if (response?.data) {                
                this.FinancialParams = response?.data?.data
                this.VpsCurrency = response?.data?.data?.currency
                this.VpsRatio = response?.data?.data?.ratio
                this.mountToolTips();
                this.loadFilterTermsForVPSPricing();
            }
            
        },

        async loadFilterTermsForVPSPricing() {
            let formData = new FormData();
            formData.append('origin', true);
            this.FilterTermsAreLoaded = false
            const response = await axios.post(this.createRequestLinkForVPSPricing('loadFilterTerms'), formData)
        ...
        async loadPlansFromFiltersTermForVPSPricing() {
            this.plansAreLoaded = false;
            this.plansAreLoading = true
            this.plans = [];
            ...
            if(Object.keys(this.selectedTerms).length === 0){
                ...
                // formData.append('term[]', 291)
                formData.append('term[][]' , 291);
                response = await axios.post(this.createRequestLinkForVPSPricing('GetPlansFromFiltersTerm'), formData)                
            } else {
                let formData = new FormData();
                for (const term in this.selectedTerms) {
                    if (this.selectedTerms.hasOwnProperty(term)) { 
        ...
        async loadRecommendedPlansForContinentForVPSPricing() {
            this.recomPlans = [];
            // check if there is country in terms to call recommends
            for (let key in this.selectedTerms) {
                if(key == 'Country'){
                    ...
                    if (Array.isArray(CountryIds) && CountryIds?.length > 0){
                        ...
                        if(this.selectedTerms['Country'] && this.selectedTerms['Country'].length > 0){
                            ...
                            // RequestLink = this.CreateRequestLink(action = 'CaasifyGetRecomPlansForContinent');
                            response = await axios.post(this.createRequestLinkForVPSPricing('GetRecomPlansForContinent'), formData)
                            // response = await axios.post(RequestLink, formData);
                        }
        ...
        createRequestLinkForVPSPricing(action){
            if(this.systemUrl == null){
                this.systemUrl = '/'
            }
            return this.systemUrl + '/modules/addons/caasify/views/view/includes/vpspricing/fetch-products.php?action=' + action
        }
    }
});

vpsapp.config.compilerOptions.isCustomElement = tag => tag === 'btn'
vpsapp.config.devtools = true;
vpsapp.mount('#vpsapp') 
